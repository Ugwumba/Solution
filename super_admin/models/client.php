...
class Account{
    ...
    public function register() {
        if (isset($_POST["action"]) && $_POST["action"] == 1) {
            ...
            if (empty($error)) {
                $firstname = $this->treat($_POST['firstname']);
                $lastname = $this->treat($_POST['lastname']);
                $email = $this->treat($_POST['email']);
                $password = $this->treat($_POST['password']);
                $password = md5(sha1($password));
                $phone = $this->treat($_POST['phone']);
                $address = $this->treat($_POST['address']);
                ...
                if ($query->rowCount() > 0) {
                    ...
                    $insertSQL = "INSERT INTO $this->table (`firstname`, `lastname`,`email`, `password`, `phone`, `address`) VALUES (?, ?, ?, ?, ?, ?)";
                    $insertQuery = $this->conn->prepare($insertSQL);
                    if ($insertQuery->execute([$firstname, $lastname, $email, $password, $phone, $address])) {
